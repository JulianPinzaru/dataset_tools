...
def makeResize(img,filename,scale,subdir):

    remakePath = args.output_folder + str(scale)+"/" + subdir + "/"
...
def makeResizeToRectangle(img,filename,width,height,allow_rotating,subdir):
    remakePath = args.output_folder + "-resize_rectangle-"+str(width)+"x"+str(height)+"/" + subdir + "/"
    if not os.path.exists(remakePath):
        os.makedirs(remakePath)
...
def makeCanny(img,filename,scale,subdir):
    make_path = args.output_folder + "canny-"+str(scale)+"/" + subdir + "/"
...
def makeCrop(img,filename,subdir):
    make_path = args.output_folder + "crop-"+str(args.height)+"x"+str(args.width)+"/" + subdir + "/"
    if not os.path.exists(make_path):
        os.makedirs(make_path)
...
def makeSquareCrop(img,filename,scale,subdir):
    make_path = args.output_folder + "sq-"+str(scale)+"/" + subdir + "/"
    if not os.path.exists(make_path):
        os.makedirs(make_path)
...
def makeManySquares(img,filename,scale,subdir):
    make_path = args.output_folder + "many_squares-"+str(scale)+"/" + subdir + "/"
    if not os.path.exists(make_path):
        os.makedirs(make_path)
...
def makeSquareCropPatch(img,filename,scale,subdir):
    make_path = args.output_folder + "sq-"+str(scale)+"/" + subdir + "/"
    if not os.path.exists(make_path):
        os.makedirs(make_path)
...
def makePix2Pix(img,filename,scale,subdir,direction="BtoA",value=[0,0,0]):
    img_p2p = img.copy()
    img_p2p = image_resize(img_p2p, max = scale)
    (h, w) = img_p2p.shape[:2]
    bType = cv2.BORDER_CONSTANT
    
    make_path = args.output_folder + "pix2pix-"+str(h)+"/" + subdir + "/"
    if not os.path.exists(make_path):
        os.makedirs(make_path)
...
def processImage(img,filename,subdir):
    ...
    if args.process_type == 'resize_to_rectangle':
        makeResizeToRectangle(img,filename,args.width,args.height,args.allow_rotating,subdir)
    ...
    if args.process_type == "crop_center_square":
        makeCropCenterSquare(img,filename,args.max_size,subdir)
    if args.process_type == "canny":
       makeCanny(img,filename,args.max_size,subdir)
    if args.process_type == "cannypix2pix":
    ...
    for root, subdirs, files in os.walk(args.input_folder):
        ...
        for filename in files:
            ...
            if hasattr(img, 'copy'):
                ...
                if args.name:
                    processImage(img,filename,current_subdir)
                else:
                    processImage(img,str(count),current_subdir)
                count = count + int(1)
